    ...
    return (
      <div style={{ maxWidth: '420px', margin: '80px auto', padding: '0 24px', textAlign: 'center' }}>
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}>
          ...
          <h2 style={{ fontSize: '24px', fontFamily: 'var(--font-display)', color: '#e8d48b', letterSpacing: '4px' }}>
            验证邮件已发送
          </h2>
          <p style={{ color: 'rgba(255,255,255,0.6)', marginTop: '12px', lineHeight: 1.6 }}>
            请检查 {email} 的收件箱，点击邮件中的链接完成验证。
          </p>
          <Link to="/login" style={{ color: '#c9a84c', marginTop: '24px', display: 'inline-block' }}>返回登录</Link>
        </motion.div>
      </div>
    );
  }
    ...
    <div style={{ maxWidth: '420px', margin: '80px auto', padding: '0 24px' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <form onSubmit={handleSubmit} style={{
          background: 'rgba(35,20,56,0.8)', borderRadius: '16px', padding: '32px',
          ...
          <label style={{ color: '#e8d48b', fontSize: '13px', letterSpacing: '1px', marginBottom: '6px', display: 'block' }}>密码（至少6位）</label>
          ...
          <p style={{ textAlign: 'center', marginTop: '16px', color: 'rgba(255,255,255,0.4)', fontSize: '13px' }}>
            已有账号？<Link to="/login" style={{ color: '#c9a84c' }}>登录</Link>
          </p>
        </form>
      </motion.div>
    </div>
