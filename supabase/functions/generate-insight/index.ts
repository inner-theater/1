...
function extractEntities(text: string): string[] {
  ...
  // 地名：模式 + 城市列表（含大部分中国城市，覆盖"武汉"等）
  const placePatterns = [
    /(?:在|去|到|从|住|移居|搬到|生活在)([\u4e00-\u9fff]{2,6}(?:市|省|县|区|镇|村|街道|路|街|广场|花园|大厦|中心))/g,
    /(?:北京|上海|广州|深圳|杭州|成都|武汉|南京|重庆|西安|长沙|苏州|天津|厦门|青岛|大连|昆明|哈尔滨|长春|沈阳|郑州|济南|合肥|福州|南昌|贵阳|南宁|海口|拉萨|银川|西宁|兰州|乌鲁木齐|香港|澳门|台北|三亚|大理|丽江|桂林|青岛|威海|烟台|泉州|温州|宁波|无锡|常州|南通|徐州|扬州|镇江|泰州|绍兴|嘉兴|金华|台州|芜湖|蚌埠|洛阳|南阳|新乡|许昌|宜昌|襄阳|荆州|株洲|湘潭|衡阳|岳阳|绵阳|宜宾|泸州|遵义|曲靖|大理|赣州|九江|上饶|宜春|乐山|自贡|南充|内江|达州|绵阳|德阳|眉山|广元|巴中|雅安|遂宁|资阳|临汾|运城|晋中|长治|吕梁|忻州|临沧|保山|普洱|昭通|玉溪|楚雄|红河|文山|西双版纳|大理州|德宏|怒江|迪庆)/g,
  ];
  // 公司/机构：词缀模式 + 知名公司扩展（教育类：猿辅导/学而思/作业帮/新东方等）
  const companyPatterns = [
    /([\u4e00-\u9fffA-Za-z0-9]{2,10}(?:公司|集团|科技|教育|网络|软件|传媒|文化|咨询|银行|证券|保险|地产|汽车|餐饮|食品|医药|能源|航空|物流|基金|研究院|培训学校|实验学校|附属中学|附属小学))/g,
    /(?:阿里|腾讯|百度|字节跳动|字节|华为|小米|比亚迪|特斯拉|苹果|谷歌|微软|亚马逊|Meta|OpenAI|蔚来|理想|小鹏|京东|美团|滴滴|拼多多|哔哩哔哩|B站|小红书|网易|快手|猿辅导|学而思|作业帮|新东方|好未来|高途|掌门|作业盒子|科大讯飞|商汤|旷视|大疆|OPPO|vivo|荣耀|中兴|联想|海尔|格力|美的|TCL|携程|飞猪|去哪儿|同程|boss直聘|智联|前程无忧|猎聘|拉勾|脉脉|知乎|微博|豆瓣|喜马拉雅|得到|极客时间|完美世界|三七互娱|米哈游|莉莉丝|鹰角)/g,
  ];
  // 岗位/职业：教育岗、互联网岗、传统职业
  const jobPatterns = [
    /(?:数学|语文|英语|物理|化学|生物|历史|地理|政治|美术|音乐|体育|科学|编程|奥数)?(?:小学|初中|高中|大学|少儿|学前)?(?:辅导|培训|托管|网课)?(?:老师|教师|讲师|助教|教练|教研)/g,
    /(?:产品|运营|市场|销售|内容|增长|用户|数据|算法|后端|前端|测试|运维|人力资源|财务|法务|行政|策划|设计|开发|架构|安全|风控|商务|采购|客服|供应链|直播|电商)?(?:工程师|经理|总监|负责人|专员|顾问|助理|实习生|组长|主管|员工|岗位|运营)/g,
    /(?:程序员|设计师|产品经理|运营|销售|教师|医生|律师|公务员|自由职业|创业|考研|考公|留学|转行|跳槽|外卖员|快递员|司机|厨师|会计|护士|工程师|研究员|记者|编辑|导演|演员|歌手|主播|健身教练|咖啡师|摄影师|自媒体|网红)/g,
  ];
  // 其他：星座、作品、食物、健康
  const zodiacPatterns = /(?:白羊座|金牛座|双子座|巨蟹座|狮子座|处女座|天秤座|天蝎座|射手座|摩羯座|水瓶座|双鱼座)/g;
  const workPatterns = /《([\u4e00-\u9fff]{1,20})》/g;
  const foodPatterns = /(?:火锅|烧烤|日料|西餐|中餐|川菜|粤菜|湘菜|淮扬菜|海鲜|素食|轻食|奶茶|咖啡|烘焙|甜品|热干面|豆皮|小龙虾|臭豆腐|螺蛳粉|麻辣烫|凉皮|肉夹馍|煎饼|烤鸭|包子|饺子|面条|米粉|米线|粥|烧烤摊|夜市|大排档)/g;
  const healthPatterns = /(?:焦虑|抑郁|失眠|头痛|胃病|过敏|高血压|糖尿病|颈椎|腰椎|近视|牙痛|感冒|发烧|咳嗽)/g;

  const allPatterns = [
    ...placePatterns, ...companyPatterns, ...jobPatterns,
    zodiacPatterns, workPatterns, foodPatterns, healthPatterns,
  ];
...
- 绝对不要做"分析式句式"——禁止"用的是..."、"这说明..."、"说明你..."、"这意味着..."、"可以看出..."、"X 类型的..."这种语文老师批改作文的腔调。你不是站在旁边评论他的文字，你是跟他在一起经历这件事。想引用他写的细节时直接共情："你写下'白云公园'那会儿，心里应该是松的"，而不是"你用的是'白云公园'这个具体的地名，这说明..."
- 最后给一个能让人松一口气的着陆：不是总结、不是金句、不是"我们都要 X"那种鸡汤。可以是接纳（"两个你都是你，你不必二选一"）、可以是共情（"这种拉扯本身说明你在乎"）、也可以是一个温柔的真相（"没走的那条不会消失，它会留在你某个周日下午的恍惚里，那不是遗憾，是证据"）。目标是让他读完把手机放下，呼一口气，而不是继续翻来覆去。`;
        break;
      }
      case 'friend-room': {
...
- 如果选项里提到具体的城市、公司、岗位，你必须像真的了解它们一样去写：
  · 城市——那个城市的生活质感（比如武汉：热干面的早、长江边的晚风、光谷的写字楼、江汉路的周末人潮；北京：通勤的地铁、望京的工区、周末逛的胡同）。
  · 公司——那个公司的日常氛围（比如字节跳动：工区里的会议室、OKR 文化、免费三餐、夜里十点还在亮的工位灯；猿辅导：教研会、家长群、续班率的压力、寒暑假的冲刺节奏）。不知道细节就写行业共通的真实感，别编造具体谣言或薪资数字。
  · 岗位——那种工作的具体日常（小学数学老师：备课、磨课、家长会、看到孩子开窍的瞬间；本地生活运营：商家拜访、数据周报、活动方案、深夜改 PPT）。
- 如果我有附上联网搜索到的资料，参考它们但不要生硬罗列，像你本来就懂一样自然融进去。
...
输出严格JSON数组：[{"q":"直面灵魂的题面","options":["A.有深度的选项","B.有深度的选项","C.有深度的选项","D.有深度的选项"]},...]`;
    ...
    // 联网搜索（generate-questions 是让 AI 出题，不需要；其余全部搜索，含 generate-letter）
    if (gameType !== 'generate-questions') {
