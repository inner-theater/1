...
serve(async (req) => {
  ...
  try {
    ...
    let temperature = 0.8;
    let maxTokens = context?.maxTokens || 500;
...
1. 从这个选择能看到他内心深处真正的优先级是什么——为什么是这个选项而不是其他？他的理性可能还在琢磨，但手指已经替心做了选择
...
1. 分析他为什么删掉那些——是因为不够怕，还是因为能承受？每一条删除都是一种"我允许"
2. 留下来的那一条就是他的底线——深入解读：他真正恐惧的不是这件事的结果，而是这件事背后的什么？比如他怕"让父母失望"，背后可能是他害怕不被认可
...
1. 从金币分配分析他的价值排序——花最多的在哪儿，一分没给的在哪儿，这揭示了他最看重的和最不在意的
2. 跳过的价值可能不是不重要，而是他潜意识在回避——结合头像风格戳他一下
3. 余额的心理学：剩余金币说明什么——是不是在等一个还没出现的选项？
4. 倾向「${context.result || ''}」和他买下的价值之间，是内在一致还是矛盾？
5. 最后用老朋友的语气说一句真正的话
...
1. 从他的答案中找出他的心理模式——矛盾或一致性。比如某道题他说"果断"，但另一道题暴露了他其实害怕后悔
2. 塔罗牌和他的答题模式之间有什么呼应——这不是占卜，是镜像
3. 他答完10道题后自己可能都没发现的模式——你作为一个"老朋友"帮他说出来
4. 最后不是鼓励，是说真话。老朋友不说漂亮话
